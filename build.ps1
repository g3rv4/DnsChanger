...
docker run --rm -v "$($basePath):/var/src" mcr.microsoft.com/dotnet/core/sdk:3.1.404-alpine3.12 ash -c "dotnet publish -c Release /var/src/DnsChanger.Web/DnsChanger.Web.csproj -o /var/src/build && chown -R $($uid):$($gid) /var/src"
